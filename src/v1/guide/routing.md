...
CatLib路由系统由`特性路由`，`路由条目`，`路由编译器`，`路由组`组成。
...
注册一个基本理由只需要一个`url`与一个`lambda`, 如果您没有定义uri的`scheme`那么将会使用默认值:
...
路由中间件在宏观面上分为：`全局中间件` ， `环境中间件` ，`区域中间件` , 中间件的执行流程为：

(调度入口) `全局中间件` => `环境中间件` => `区域中间件` => `目标路由方法`
(调度出口) `全局中间件` <= `环境中间件` <= `区域中间件` <= `目标路由方法`

> `全局中间件` 和 `环境中间件` , `区域中间件` 为不同的过滤器链池所以她们之间的`优先级`不产生影响。

### 特性路由中间件

路由条目中间件属于`区域中间件`。

通过特性路由的方式进行路由，将允许您设定类的中间件接口：`IMiddleware`;
...
异常处理总是从`局部`开始的，CatLib的异常处理器会依次调用异常处理函数，逐层冒泡执行。
...
在递归调用的情况下，全局异常处理只会执行一次。

假设我们进行了一次递归调用,异常将会按照如下流程触发：

`触发异常` => `局部异常处理(嵌套的Dispatch("ui://call2"))` => `局部异常处理(ui://call)` => `全局异常处理` => `Dispatch()抛出异常`
